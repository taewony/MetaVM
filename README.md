...
MetaVM 프로젝트는 **AI 통합/협업 프로그래밍 모델 혹은 AI 중심 개발 프로세스**에 대한 연구와 pilot 개발을 목표로 합니다. 이 프로젝트는 다음과 같은 구성 요소를 갖춘 실험적인 프레임워크를 제공합니다:

* **MiniLang:** 명확한 데이터 흐름 의미론을 통해 데이터 분석/처리 작업을 표현하고 작업을 자동화하는 데 중점을 둔 맞춤형 **DSL(도메인 특화 언어)**입니다.
* **AI 통합:** **LLM(대규모 언어 모델)**과 상호작용을 통해 코드 생성(자연어 -> MiniLang) 변환을 하고, 작성된 mini DSL 언어를 코드로 변환하며, 최대한 자동으로 실행하는 workflow를 목표로 합니다. 특히 **실행 피드백 루프**를 통해 AI가 코드를 개선하고 학습하는 독창적인 기능을 제공할 수 있습니다.
* **MetaVM 컴파일러/에뮬레이터 & IDE:** MetaVM 생태계 내에서 MiniLang 코드의 개발, 실행 및 분석을 지원하는 도구 지원. AI 중심의 새로운 개발 워크플로우를 가능하게 하는 핵심 인프라입니다.

**핵심 비전:**

1.  **데이터 처리/분석에 특화된 mini DSL 언어 설계**
2.  **이러한 mini DSL을 통한 workflow를 LLM과 연동**
3.  **엣지 서버나 특수 하드웨어를 대상으로 하는 효율적인 실행 모델을 탐구**
